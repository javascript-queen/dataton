...
- ```transformer_net.py``` -- это архитектура, целью которой является решение последовательных задач при обработке долгосрочных зависимостей

- ```utils.py``` -- утилиты для обучения модели

- ```vgg.py``` -- файл под лицензией Pytorch. VGG означает группу визуальной геометрии; это стандартная архитектура глубокой сверточной нейронной сети (CNN) с несколькими уровнями.
